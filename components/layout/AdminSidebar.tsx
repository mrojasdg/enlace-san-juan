        ...
        <aside className="w-[300px] h-screen bg-green-deeper text-white flex flex-col items-center justify-between py-12 px-6 border-r border-white/5 relative z-50 shadow-2xl overflow-hidden shrink-0">
            ...
            <div className="w-full flex flex-col items-center gap-12 relative z-10">
                <Link href="/admin/dashboard" className="group">
                    ...
                    <div className="mt-4 flex items-center justify-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/10">
                        ...
                        <span className="text-[9px] font-black uppercase tracking-[0.25em] text-white/40">Admin Panel</span>
                    </div>
                </Link>

                {/* Navigation Menu */}
                <nav className="w-full space-y-2">
                    <p className="px-4 text-[9px] font-black uppercase tracking-[0.2em] text-white/20 mb-4">Menú principal</p>
                    {menuItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <item.icon size={22} className={cn("transition-transform group-hover:scale-110", isActive ? "text-green-mid" : "text-white/20")} />
                ...
                <nav className="w-full space-y-2 mt-8">
                    <p className="px-4 text-[9px] font-black uppercase tracking-[0.2em] text-white/20 mb-4">Sistema</p>
                    {secondaryItems.map((item) => (
                        ...
                        >
                            <item.icon size={18} className="text-white/10 group-hover:text-white/30" />
            ...
            <div className="w-full space-y-6 relative z-10 pt-12 border-t border-white/5">
                <div className="flex items-center gap-4 px-2">
                    ...
                    <div>
                        <p className="text-xs font-black text-white leading-tight uppercase tracking-widest">Administrador</p>
                        <p className="text-[10px] text-white/40 leading-tight">Acceso total</p>
                    </div>
                </div>
