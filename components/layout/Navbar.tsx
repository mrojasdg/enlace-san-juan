...
import { Menu, X, Search as SearchIcon, Loader2, ArrowRight } from "lucide-react";
        ...
        >
            <div className="container mx-auto px-6 md:px-12 flex items-center justify-between gap-8">
                ...
                {showSearch && (
                    ...
                    >
                        <SearchIcon size={16} className="text-muted/60 group-focus-within:text-green mr-3" />
                        <input
                            type="text"
                        ...
                        />
                        <button type="submit" disabled={isSearching} className="ml-2 bg-green px-4 py-2 rounded-full text-white hover:bg-green-mid transition-all disabled:opacity-50 flex items-center justify-center min-w-[36px] min-h-[36px]">
                            {isSearching ? <Loader2 size={14} className="animate-spin text-white" /> : <SearchIcon size={14} className="text-white" />}
                        </button>
                    </form>
                )}
                ...
                <div className="hidden lg:flex items-center gap-4">
                    <Link href="/admin/login">
                        <Button variant="outline" size="sm" className="rounded-xl border-border px-6 h-10 text-[10px] font-black uppercase tracking-widest text-muted">
                            Panel
                        </Button>
                    </Link>
                    <Link href="/registrate">
                        <Button size="sm" className="rounded-xl h-10 px-6 text-[10px] font-black uppercase tracking-widest bg-green hover:bg-green-mid border-none shadow-lg shadow-green/20">
                            Registra tu negocio
                        </Button>
                    </Link>
                </div>
                    ...
                    <div className="flex flex-col p-6 gap-6">
                        ...
                        {showSearch && (
                            ...
                            >
                                ...
                                <input
                                    ...
                                    autoFocus
                                />
                                <button type="submit" disabled={isSearching} className="ml-3 bg-green p-3 rounded-xl text-white hover:bg-green-mid transition-all disabled:opacity-50 flex items-center justify-center min-w-[44px] min-h-[44px]">
                                    {isSearching ? <Loader2 size={18} className="animate-spin text-white" /> : <SearchIcon size={18} className="text-white" />}
                                </button>
                            </form>
                        )}
                        ...
                        <div className="grid grid-cols-1 gap-1">
                            {navLinks.map((link) => (
                                ...
                                >
                                    {link.name}
                                    <SearchIcon size={14} className="opacity-0 group-hover:opacity-100 transition-all text-green" />
                                </Link>
                            ))}
                        </div>
                        ...
                        <div className="flex flex-col gap-3 mt-4">
                            <Link href="/admin/login" onClick={() => setIsOpen(false)}>
                                <Button variant="outline" className="w-full h-14 rounded-2xl border-border font-black uppercase text-xs tracking-widest text-muted">
                                    Panel de Control
                                </Button>
                            </Link>
