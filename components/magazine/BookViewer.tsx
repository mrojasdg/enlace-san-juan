...
import { ArrowLeft, ArrowRight, X, ZoomIn, ZoomOut, Maximize, Move, ExternalLink } from "lucide-react";
...
export const BookViewer = ({ pages, total }: BookViewerProps) => {
    ...
    const renderPage = (num: number, side: 'left' | 'right' | 'single' = 'single') => {
        ...
        return (
            <div
                className={`relative h-full aspect-[9/16] bg-[#111] shadow-2xl transition-all duration-300
                    ${side === 'left' ? 'rounded-l-xl border-y border-l border-white/5' : ''}
                    ${side === 'right' ? 'rounded-r-xl border-y border-r border-white/5' : ''}
                    ${side === 'single' ? 'rounded-xl border border-white/5' : ''}
                `}
            >
                <div className="w-full h-full overflow-hidden rounded-[inherit]">
                    {data?.image_url ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-white/5 bg-black/50">
                            <span className="font-outfit text-5xl md:text-7xl font-black opacity-30">{num}</span>
                        </div>
                    )}
                </div>
                ...
                {/* Sombra lomo */}
                {side === 'left' && <div className="absolute inset-y-0 right-0 w-12 bg-gradient-to-l from-black/50 to-transparent pointer-events-none z-20" />}
                {side === 'right' && <div className="absolute inset-y-0 left-0 w-12 bg-gradient-to-r from-black/50 to-transparent pointer-events-none z-20" />}
            </div>
        );
    };
        ...
        <div className="fixed inset-0 w-full h-[100dvh] bg-[#050706] flex flex-col items-center justify-start overflow-hidden select-none">
            ...
            `}>
                ...
                <div
                    ...
                    style={{
                        ...
                        transition: isDragging ? 'none' : 'transform 0.4s cubic-bezier(0.2, 0, 0.4, 1), opacity 0.3s ease'
                    }}
                ...
                >
                    <div className={`flex items-center justify-center max-w-full gap-0 ${isDesktop ? 'h-[85vh]' : 'h-[82vh]'}`}>
                ...
                {/* Desktop Nav Buttons */}
                <button onClick={handlePrev} disabled={slideIndex === 0 || isChanging} className="absolute left-8 top-1/2 -translate-y-1/2 z-[100] p-4 bg-black/40 hover:bg-green text-white rounded-full backdrop-blur-md transition-all disabled:opacity-0 disabled:pointer-events-none border border-white/10 hidden md:block shadow-2xl">
                    <ArrowLeft className="w-8 h-8" />
                </button>
                <button onClick={handleNext} disabled={slideIndex === slides.length - 1 || isChanging} className="absolute right-8 top-1/2 -translate-y-1/2 z-[100] p-4 bg-black/40 hover:bg-green text-white rounded-full backdrop-blur-md transition-all disabled:opacity-0 disabled:pointer-events-none border border-white/10 hidden md:block shadow-2xl">
            ...
            <div className="absolute bottom-0 left-0 w-full h-[80px] md:h-[90px] bg-black/95 backdrop-blur-xl border-t border-white/5 flex items-center justify-between px-4 md:px-12 z-[200]">
                ...
                <div className="flex items-center gap-2 md:gap-4 md:w-1/3">
                    <button onClick={handlePrev} disabled={slideIndex === 0 || isChanging} className="p-2 md:p-3 bg-white/5 rounded-full hover:bg-green text-white transition-all disabled:opacity-0 border border-white/5">
                    ...
                    <div className="text-center min-w-[80px] md:min-w-[100px]">
                        <span className="text-white font-mono font-bold text-xs md:text-lg tracking-tighter">
                            {currentSlide.length > 1 ? `${currentSlide[0]}-${currentSlide[1]}` : currentSlide[0]}
                            <span className="text-white/30 mx-1.5">/</span>
                            {total}
                        </span>
                    </div>
                    <button onClick={handleNext} disabled={slideIndex === slides.length - 1 || isChanging} className="p-2 md:p-3 bg-white/5 rounded-full hover:bg-green text-white transition-all disabled:opacity-0 border border-white/5">
                ...
                <div className="hidden md:flex justify-center md:w-1/3">
                    <Link href="/">
                        <img src="/logo-white.png" alt="Logo" className="h-7 opacity-40 hover:opacity-100 transition-opacity" />
                    </Link>
                </div>

                {/* Controles */}
                <div className="flex items-center gap-1.5 md:gap-3 justify-end md:w-1/3">
                    <button onClick={handleZoomOut} disabled={scale <= 1} className="p-2 md:p-2.5 bg-white/5 text-white rounded-full hover:bg-white/10 transition-all disabled:opacity-20 border border-white/5">
                        <ZoomOut size={16} className="md:w-5 md:h-5" />
                    </button>
                    <button onClick={handleZoomIn} disabled={scale >= 3} className="p-2 md:p-2.5 bg-white/5 text-white rounded-full hover:bg-white/10 transition-all disabled:opacity-20 border border-white/5">
