...
const MapPicker = dynamic(() => import("./MapPicker").then(mod => mod.default), {
    ssr: false,
    loading: () => <div className="h-96 w-full bg-green-xpale animate-pulse rounded-3xl flex items-center justify-center text-muted font-black uppercase text-[10px] tracking-widest">Cargando Mapa...</div>
});
...
const businessSchema = z.object({
    ...
    description: z.string().min(10, "Mínimo 10 caracteres").max(800, "Máximo 800 caracteres"),
...
export const BusinessForm = ({ initialData, categories, isPublicRegistration = false }: BusinessFormProps) => {
            ...
            <form onSubmit={handleSubmit(onSubmit, onFormError)} className="p-10 md:p-14">
                    ...
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div>
                            <h2 className="font-outfit font-black text-3xl text-green-deeper mb-4 flex items-center gap-4">
                                <span className="w-10 h-10 rounded-2xl bg-green text-white flex items-center justify-center text-lg">1</span>
                                Información básica
                            </h2>
                            <p className="text-muted text-sm font-jakarta max-w-lg">Define el nombre atractivo y la descripción que enganchará a tus clientes.</p>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                            <div className="space-y-2 group">
                                <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block transition-colors group-focus-within:text-green">Nombre comercial</label>
                                <input {...register("name")} placeholder="Ej: Pizzas Don Pepe" className={cn("w-full bg-green-xpale border border-border focus:bg-white focus:border-green focus:shadow-xl outline-none rounded-2xl py-5 px-6 font-bold text-ink transition-all placeholder:opacity-20", errors.name && "border-red-500")} />
                            </div>
                            <div className={cn("space-y-2 group", isPublicRegistration && "hidden")}>
                                <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block">SLUG URL</label>
                                <div className="relative">
                                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-muted/30 text-[12px] font-bold">/</span>
                                    <input {...register("slug")} className="w-full bg-green-pale/30 border border-border focus:bg-white focus:border-green outline-none rounded-2xl py-5 pl-8 pr-6 font-mono text-[11px] font-bold text-ink2 transition-all" />
                                </div>
                            </div>
                            <div className="md:col-span-2 space-y-2 group">
                                <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block">Eslogan / Frase corta</label>
                                <input {...register("tagline")} placeholder="La mejor cocina italiana..." className="w-full bg-green-xpale border border-border focus:bg-white focus:border-green outline-none rounded-2xl py-5 px-6 font-bold text-ink transition-all" />
                            </div>
                            <div className="md:col-span-2 space-y-2 group">
                                <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block">Descripción</label>
                                <textarea {...register("description")} rows={5} placeholder="Describe qué hace único a tu negocio..." className="w-full bg-green-xpale border border-border focus:bg-white focus:border-green outline-none rounded-2xl py-5 px-6 font-bold text-ink transition-all" />
                            </div>
                            <div className={cn("md:col-span-2 space-y-2 group", isPublicRegistration && "hidden")}>
                                <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block flex items-center gap-2">Palabras Clave de Búsqueda (Solo Interno) <Info size={12} className="text-blue-500"/></label>
                                <input {...register("search_keywords")} placeholder="tacos, garnachas, comida rapida, economico" className="w-full bg-blue-50/50 border border-blue-100 focus:bg-white focus:border-blue-400 outline-none rounded-2xl py-4 px-6 font-bold text-ink transition-all placeholder:opacity-50" />
                                <p className="text-[9px] text-muted font-bold ml-2">Agrega palabras separadas por comas. Estas palabras ayudarán a que el negocio aparezca en las búsquedas, pero no serán visibles para el público.</p>
                            </div>
                            ...
                            <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-10 p-6 bg-red-50/50 border border-red-100/50 rounded-3xl mt-4 mb-2">
                                <div className="md:col-span-3 flex items-center gap-3 mb-2">
                                    ...
                                    <div>
                                        <h4 className="font-outfit font-black text-ink text-sm">Datos del Representante (Privado)</h4>
                                        <p className="text-[10px] text-muted font-bold tracking-widest uppercase">Solo visible en administración</p>
                                    </div>
                                </div>
                                <div className="space-y-2 group">
                                    <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block transition-colors group-focus-within:text-red-500">Nombre del Contacto</label>
                                    <input {...register("contact_name")} placeholder="Nombre completo" className="w-full bg-white border border-border focus:border-red-400 outline-none rounded-2xl py-5 px-6 font-bold text-ink transition-all" />
                                </div>
                                <div className="space-y-2 group">
                                    <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block transition-colors group-focus-within:text-red-500">Email del Contacto</label>
                                    <input type="email" {...register("contact_email")} placeholder="correo@ejemplo.com" className="w-full bg-white border border-border focus:border-red-400 outline-none rounded-2xl py-5 px-6 font-bold text-ink transition-all" />
                                </div>
                                <div className="space-y-2 group">
                                    <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block transition-colors group-focus-within:text-red-500">WhatsApp / Teléfono</label>
                                    <input type="tel" {...register("contact_phone")} placeholder="427 123 4567" className="w-full bg-white border border-border focus:border-red-400 outline-none rounded-2xl py-5 px-6 font-bold text-ink transition-all" />
                                </div>
                            </div>
                            <div className="md:col-span-2 space-y-2 group">
                                <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block">Sitio Web Oficial</label>
                                <input {...register("website")} className="w-full bg-green-xpale border border-border outline-none rounded-2xl py-4 px-6 font-bold text-ink" placeholder="https://mi-empresa.com" />
                            </div>
                            <div className="space-y-2 group">
                                <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block">Categoría</label>
                                <select {...register("category_id")} className="w-full bg-green-xpale border border-border focus:bg-white focus:border-green outline-none rounded-2xl py-5 px-6 font-bold text-ink appearance-none cursor-pointer">
                            ...
                            <div className={cn("md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8", isPublicRegistration && "hidden")}>
                                <div className="space-y-2 group flex flex-col">
                                    <label className="text-[10px] font-black text-green uppercase tracking-[0.2em] ml-2 block">Estado de Visibilidad</label>
                                    <div className="flex items-center gap-4 bg-green-xpale/50 border border-green/20 rounded-2xl py-4 px-6 h-full transition-all hover:bg-white hover:shadow-lg">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-[10px] text-ink font-black uppercase tracking-tight">Autorización</p>
                                            <p className="text-[9px] text-muted font-bold leading-tight">
                                                Si está oculto, no aparecerá en el directorio ni en búsquedas.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2 group flex flex-col">
                                    <label className="text-[10px] font-black text-green uppercase tracking-[0.2em] ml-2 block">Destacar Empresa</label>
                                    <div className="flex items-center gap-4 bg-green-xpale/50 border border-green/20 rounded-2xl py-4 px-6 h-full transition-all hover:bg-white hover:shadow-lg">
                                        <Toggle
                                        ...
                                        />
                                        <div className="flex-1">
                                            <p className="text-[10px] text-ink font-black uppercase tracking-tight">Carrusel Inicio</p>
                                            <p className="text-[9px] text-muted font-bold leading-tight">
                                        ...
                                        <div className={cn(
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="text-[9px] text-muted font-bold uppercase tracking-widest mt-1">
                                                    {pdfFile ? `${(pdfFile.size / (1024 * 1024)).toFixed(2)} MB` : "Máximo 10MB"}
                                                </p>
                                            </div>
                                            {(pdfFile || pdfUrl) && (
                                                <button
                    ...
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            ...
                            <div className="space-y-6">
                                <label className="text-[10px] font-black text-muted uppercase tracking-widest ml-2 block text-center">Logo</label>
                                <div className="relative aspect-square max-w-[200px] mx-auto rounded-full bg-green-xpale border-2 border-dashed border-border flex items-center justify-center overflow-hidden hover:border-green transition-colors group cursor-pointer shadow-xl">
                                    {logoPreview ? (
                                        ...
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="font-outfit font-black text-green-deeper uppercase tracking-widest text-[9px]">Logo</p>
                                        </div>
                                    )}
                                    <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" accept="image/*" onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                                        const file = e.target.files?.[0];
                            ...
                            {/* Cover */}
                            <div className="space-y-6">
                                <label className="text-[10px] font-black text-muted uppercase tracking-widest ml-2 block">Imagen de portada (que sea horizontal)</label>
                                <div className="relative aspect-video rounded-[2.5rem] bg-green-xpale border-2 border-dashed border-border flex items-center justify-center overflow-hidden hover:border-green transition-colors group cursor-pointer shadow-xl">
                                    {coverPreview ? (
                                        ...
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="font-outfit font-black text-green-deeper uppercase tracking-widest text-[11px]">Subir Portada</p>
                                        </div>
                                    )}
                                    <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" accept="image/*" onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                                        const file = e.target.files?.[0];
                        ...
                        <div className="space-y-6">
                            <div className="flex justify-between items-center px-2">
                                ...
                                <span className="text-[10px] font-bold text-muted">{existingGallery.length + galleryFiles.length} / 20</span>
                            </div>
                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                ...
                                {existingGallery.length + galleryFiles.length < 20 && (
                                    <label className="aspect-square rounded-3xl bg-green-xpale border-2 border-dashed border-border flex flex-col items-center justify-center cursor-pointer hover:border-green transition-all group shadow-sm">
                                        <Plus size={24} className="text-green-light mb-2 group-hover:scale-110 transition-transform" />
                                        <span className="text-[9px] font-black text-green-deeper uppercase tracking-widest">Añadir</span>
                                ...
                                {existingGallery.map((url, i) => (
                                    <div key={`existing-${i}`} className="aspect-square relative rounded-3xl overflow-hidden border border-border group shadow-sm">
                                        <Image src={url} alt={`Gallery ${i}`} fill className="object-cover" />
                                        <button type="button" onClick={() => removeGalleryFile(i, true)} className="absolute top-2 right-2 w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                ))}
                                ...
                                {galleryPreviews.map((url, i) => (
                                    <div key={`new-${i}`} className="aspect-square relative rounded-3xl overflow-hidden border-2 border-green group shadow-sm">
                                        <Image src={url} alt={`Preview ${i}`} fill className="object-cover" />
                                        <div className="absolute top-2 left-2 bg-green text-white text-[8px] font-black px-2 py-1 rounded-full uppercase tracking-widest">Nueva</div>
                                        <button type="button" onClick={() => removeGalleryFile(i, false)} className="absolute top-2 right-2 w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                ))}
                ...
                {activeTab === "contact" && (
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            <div className="space-y-2 group">
                                <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block">Teléfono</label>
                                <input {...register("phone")} className="w-full bg-green-xpale border border-border outline-none rounded-2xl py-4 px-6 font-bold text-ink" placeholder="427 123 4567" />
                            </div>
                            <div className="space-y-2 group">
                                <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block">WhatsApp</label>
                                <input {...register("whatsapp")} className="w-full bg-green-xpale border border-border outline-none rounded-2xl py-4 px-6 font-bold text-ink" placeholder="427 123 4567" />
                            </div>
                            <div className="space-y-2 group">
                                <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block">Email Público</label>
                                <input type="email" {...register("email")} className="w-full bg-green-xpale border border-border outline-none rounded-2xl py-4 px-6 font-bold text-ink" placeholder="contacto@empresa.com" />
                            </div>
                            <div className="md:col-span-2 lg:col-span-3 space-y-2 group">
                                <label className="text-[10px] font-black text-muted uppercase tracking-[0.2em] ml-2 block">Dirección</label>
                                <input {...register("address")} className="w-full bg-green-xpale border border-border outline-none rounded-2xl py-4 px-6 font-bold text-ink" placeholder="Calle Hidalgo #10, Centro" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-6 pt-4">
                            <div>
                                <h3 className="font-outfit font-black text-xl text-green-deeper mb-2">Ubicación en el mapa</h3>
                                <p className="text-muted text-xs font-jakarta">Arrastra el pin o haz click para definir la ubicación exacta que verán los clientes.</p>
                            </div>
                            <MapPicker
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-white border border-border rounded-xl p-4 flex flex-col gap-1">
                                    <span className="text-[8px] font-black text-muted uppercase tracking-widest">Latitud</span>
                                    <span className="text-xs font-bold font-mono text-green">{watch("latitude")?.toFixed(6) || "N/A"}</span>
                                </div>
                                <div className="bg-white border border-border rounded-xl p-4 flex flex-col gap-1">
                                    <span className="text-[8px] font-black text-muted uppercase tracking-widest">Longitud</span>
                                    <span className="text-xs font-bold font-mono text-green">{watch("longitude")?.toFixed(6) || "N/A"}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {activeTab === "extra" && (
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="grid grid-cols-1 gap-12">
                            ...
                            <div className="space-y-6">
                                <div className="px-2">
                                    <h3 className="text-xs font-black text-green uppercase tracking-widest mb-2">Servicios y características del negocio</h3>
                                    <p className="text-muted text-[10px] font-medium font-jakarta">Selecciona los servicios que ofrece tu negocio para que aparezcan en el micrositio.</p>
                                </div>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-[9px] text-muted font-bold uppercase tracking-[0.05em]">Servicio disponible</p>
                                                </div>
                                                <div className={cn(
                                                    "w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all",
                                                    isSelected ? "bg-green border-green" : "border-border group-hover:border-green-pale"
                                                )}>
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        if (checked) {
                                                            setValue("features", [...currentFeatures, feature.id]);
                                                        } else {
                                                            setValue("features", currentFeatures.filter(id => id !== feature.id));
                                                        }
                                                    }}
                                                />
                                            </label>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <h3 className="text-xs font-black text-green uppercase tracking-widest px-2 mb-1">Redes Sociales</h3>
                                    <p className="text-[10px] font-medium text-muted font-jakarta px-2">(agrega los links de tus redes sociales que si esten activas, las demás dejalas vacías)</p>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="relative group">
                                        <Facebook className="absolute left-4 top-1/2 -translate-y-1/2 text-muted/40 group-focus-within:text-blue-600 transition-colors" size={18} />
                                        <input {...register("facebook")} placeholder="Facebook URL" className="w-full bg-green-xpale border border-border rounded-2xl py-4 pl-12 pr-6 font-bold text-sm" />
                                    </div>
                                    <div className="relative group">
                                        <Instagram className="absolute left-4 top-1/2 -translate-y-1/2 text-muted/40 group-focus-within:text-pink-600 transition-colors" size={18} />
                                        <input {...register("instagram")} placeholder="Instagram URL" className="w-full bg-green-xpale border border-border rounded-2xl py-4 pl-12 pr-6 font-bold text-sm" />
                                    </div>
                                    <div className="relative group">
                                        <Twitter className="absolute left-4 top-1/2 -translate-y-1/2 text-muted/40 group-focus-within:text-ink transition-colors" size={18} />
                                        <input {...register("twitter_x")} placeholder="Twitter URL" className="w-full bg-green-xpale border border-border rounded-2xl py-4 pl-12 pr-6 font-bold text-sm" />
                                    </div>
                                    <div className="relative group">
                                        <Youtube className="absolute left-4 top-1/2 -translate-y-1/2 text-muted/40 group-focus-within:text-red-600 transition-colors" size={18} />
                                        <input {...register("youtube")} placeholder="YouTube URL" className="w-full bg-green-xpale border border-border rounded-2xl py-4 pl-12 pr-6 font-bold text-sm" />
                                    </div>
                                    <div className="relative group">
                                        <Tiktok className="absolute left-4 top-1/2 -translate-y-1/2 text-muted/40 group-focus-within:text-black transition-colors" size={18} />
                                        <input {...register("tiktok")} placeholder="TikTok URL" className="w-full bg-green-xpale border border-border rounded-2xl py-4 pl-12 pr-6 font-bold text-sm" />
                                    </div>
                                    <div className="relative group">
                                        <Pinterest className="absolute left-4 top-1/2 -translate-y-1/2 text-muted/40 group-focus-within:text-red-600 transition-colors" size={18} />
                                        <input {...register("pinterest")} placeholder="Pinterest URL" className="w-full bg-green-xpale border border-border rounded-2xl py-4 pl-12 pr-6 font-bold text-sm" />
                                    </div>
                                    <div className="relative group">
                                        <Linkedin className="absolute left-4 top-1/2 -translate-y-1/2 text-muted/40 group-focus-within:text-blue-700 transition-colors" size={18} />
                                        <input {...register("linkedin")} placeholder="LinkedIn URL" className="w-full bg-green-xpale border border-border rounded-2xl py-4 pl-12 pr-6 font-bold text-sm" />
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="mt-12 pt-8 border-t border-border flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-3">
                        <div className={cn("w-2 h-2 rounded-full", initialData ? "bg-green" : "bg-orange-400")} />
                        <span className="text-[10px] font-black uppercase text-muted tracking-widest">{initialData ? "Actualizar" : "Nuevo"}</span>
                    </div>
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        <Button type="button" variant="outline" onClick={() => router.back()} className="px-8 py-4 rounded-xl text-xs uppercase">Cancelar</Button>
                        <Button type="submit" disabled={loading} className="px-10 py-4 rounded-xl font-black uppercase tracking-widest flex-1 md:flex-none">
