...
export const SearchResults = ({ businesses, total, isLoading }: SearchResultsProps) => {
    if (isLoading) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center bg-green-xpale/20">
                ...
                <p className="font-jakarta font-bold text-sm text-green animate-pulse">Buscando negocios...</p>
            </div>
        );
    }
        ...
        <div className="flex-1 h-full bg-green-xpale/50 flex flex-col items-center overflow-y-auto w-full">
            ...
            <div className="sticky top-0 z-10 w-full bg-white/80 backdrop-blur-md px-8 py-4 border-b border-border flex justify-between items-center shadow-sm">
                <p className="font-jakarta font-bold text-sm text-ink2">
                    Mostrando <span className="text-green">{businesses.length}</span> de {total} resultados
                </p>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-xs font-black font-outfit text-ink2">Página 1</span>
                    <button className="p-2 rounded-xl bg-green-pale text-green hover:bg-green hover:text-white transition-all duration-200 disabled:opacity-30 disabled:pointer-events-none">
            ...
            <div className="w-full max-w-4xl p-6 flex flex-col gap-6">
                {businesses.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-32 text-center">
                        ...
                        <h3 className="font-outfit font-black text-2xl text-green-deeper mb-3">No hay resultados</h3>
                ...
                <div className="py-12 flex flex-col items-center gap-4">
                    <div className="flex items-center gap-2">
                        {[1, 2, 3].map((page) => (
                            <button
                                key={page}
                                className={`w-10 h-10 rounded-xl font-bold font-outfit text-sm transition-all duration-200 ${page === 1 ? "bg-green text-white shadow-xl" : "bg-white text-muted hover:bg-green-pale"
                                    }`}
                            >
                                {page}
                            </button>
