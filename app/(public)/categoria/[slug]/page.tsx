            ...
            <section className="relative py-20 overflow-hidden">
                ...
                <div className="container mx-auto px-6 relative">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                        <div>
                            <span className="text-green font-black text-xs uppercase tracking-[0.3em] mb-4 block">Directorio de San Juan del Río</span>
                            ...
                            <p className="mt-6 text-muted font-jakarta text-lg max-w-xl">
                                Explora los mejores establecimientos de {category.name.toLowerCase()} en San Juan del Río. Calidad y servicio local a tu alcance.
                            </p>
                        </div>
                        <div className="bg-white p-8 rounded-[2.5rem] border border-border shadow-xl min-w-[200px] text-center">
                            <span className="block text-4xl font-outfit font-black text-green mb-1">{businesses?.length || 0}</span>
                            <span className="text-[10px] font-black text-muted uppercase tracking-widest">Negocios activos</span>
                        </div>
                    </div>
                </div>
            </section>
                ...
                <div className="container mx-auto px-6">
                    {businesses && businesses.length > 0 ? (
                        ...
                        <div className="bg-white rounded-[3rem] p-20 text-center border-2 border-dashed border-border">
                            ...
                            <h3 className="text-2xl font-outfit font-black text-ink mb-4">Aún no hay negocios aquí</h3>
                            <p className="text-muted font-jakarta max-w-md mx-auto mb-10">
                                Estamos trabajando para traer los mejores lugares de esta categoría. ¿Tienes un negocio? ¡Sé el primero en aparecer!
                            </p>
                            <Link href="/#registrar">
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  const normalizedBusinesses = (businesses || []).map((biz: any) => ({
    ...biz,
    category: Array.isArray(biz.category) ? biz.category[0] : biz.category
  }));

  const businessCount = normalizedBusinesses.length;
  ...
  return (
    <main className="min-h-screen pt-20 bg-green-xpale/30">
      ...
      <section className="py-20">
        <div className="container mx-auto px-6">
          {normalizedBusinesses && normalizedBusinesses.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
