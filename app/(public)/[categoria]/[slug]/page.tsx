...
<<<<<<< HEAD
export default async function BusinessMicrosite({ params }: { params: { categoria: string, slug: string } }) {
    ...
    if (business.category?.slug && business.category.slug !== params.categoria) {
        redirect(`/${business.category.slug}/${business.slug}`);
    }

    const isOpen = business.schedule ? isOpenNow(business.schedule as any) : false;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div className="lg:col-span-2 space-y-16">
                            ...
                            <div className="relative h-[320px] md:h-[400px] w-full bg-green-pale">
                                {business.cover_url ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center opacity-20">
                                        <Image src="https://enlacesanjuan.com.mx/wp-content/uploads/2025/12/enlaceLogoSanJuan-300x125.png" alt="Logo" width={200} height={40} />
                                    </div>
                                )}
                            ...
                            <div className="px-8 pb-10 -mt-20 relative z-10 flex flex-col md:flex-row items-end gap-6 text-center md:text-left">
                                ...
                                <div className="w-44 h-44 rounded-full bg-white border-4 border-white shadow-2xl overflow-hidden flex-shrink-0 relative group mx-auto md:mx-0">
                                    <div className="relative w-full h-full bg-white flex items-center justify-center rounded-full overflow-hidden">
                                        {business.logo_url ? (
                                            ...
                                            <span className="font-outfit font-black text-5xl text-green/10">{business.name[0]}</span>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 pb-4">
                                    ...
                                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                                        <p className="text-muted md:text-white/90 font-medium text-xl italic leading-tight max-w-lg">{business.tagline}</p>
                                        <span className="bg-green-xpale text-green px-4 py-1.5 rounded-full font-black text-[11px] uppercase tracking-[0.2em] inline-block shadow-sm">
                                            {business.category?.name || "Premium"}
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col gap-4">
                            {business.catalog_pdf_url && (
                                <Link href={business.catalog_pdf_url} target="_blank" className="w-full">
                                    <Button size="lg" className="w-full bg-green hover:bg-green-mid text-white border-none flex items-center justify-center gap-4 rounded-2xl shadow-xl h-20 text-xl font-black tracking-tight transition-all">
                                        <FileText size={32} />
                            ...
                            <div className="flex flex-wrap gap-4">
                                <Link href={`https://wa.me/52${business.whatsapp}`} target="_blank" className="flex-1 min-w-[240px]">
                                    <Button size="lg" className="w-full bg-[#25D366] hover:bg-[#128C7E] border-none flex items-center justify-center gap-4 rounded-2xl shadow-xl h-20 text-xl font-black tracking-tight text-white">
                                ...
                                <div className="flex gap-4 flex-1 min-w-[240px]">
                                    <Link href={`tel:${business.phone}`} className="flex-1">
                                        <Button variant="outline" size="lg" className="w-full h-20 border-border hover:bg-white text-ink rounded-2xl shadow-lg">
                                            <Phone size={24} className="text-green-mid" />
                                            <span className="ml-3 font-black uppercase text-sm tracking-widest">Llamar</span>
                                        </Button>
                                    </Link>
                                </div>
                            </div>
                            {business.website && (
                                <Link href={business.website.startsWith('http') ? business.website : `https://${business.website}`} target="_blank" className="w-full">
                                    <Button variant="outline" size="lg" className="w-full h-20 border-border hover:bg-green-xpale text-ink hover:text-green-deeper rounded-2xl shadow-lg transition-all flex items-center justify-center gap-3">
                                        <Globe size={24} className="text-green-mid" />
                                        <span className="font-black uppercase text-sm tracking-widest mt-1">Visitar Sitio Web Oficial</span>
                                    </Button>
                                </Link>
                            )}
                        </div>
                        ...
                        <section className="bg-white rounded-[1.5rem] p-10 border border-border shadow-sm relative overflow-hidden">
                            ...
                            <h2 className="font-outfit font-black text-xl text-green-deeper mb-6 uppercase tracking-[0.2em]">Descripción</h2>
                        ...
                        {business.features && business.features.length > 0 && (
                            <section className="bg-white rounded-[1.5rem] p-10 border border-border shadow-sm">
                                <h2 className="font-outfit font-black text-xl text-green-deeper mb-8 uppercase tracking-[0.2em]">Servicios del negocio</h2>
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                                    {ALL_FEATURES.filter(f => business.features?.includes(f.id)).map((feature) => (
                        ...
                        {business.gallery_urls?.length > 0 && (
                            <BusinessGallery images={business.gallery_urls} businessName={business.name} />
                        )}
                        ...
                        <div className="grid grid-cols-1 gap-8">
                            ...
                            <div className="bg-white rounded-[1.5rem] p-10 border border-border shadow-xl">
                                <h3 className="font-outfit font-black text-lg text-green-deeper uppercase tracking-[0.2em] mb-6 leading-tight">
                                    Redes sociales de <br /><span className="text-green">{business.name}</span>
                                </h3>
                                <div className="flex flex-wrap gap-4">
                                    {[
                                        { icon: Facebook, key: "facebook", color: "hover:bg-[#1877F2] hover:border-[#1877F2] hover:text-white" },
                                        { icon: Instagram, key: "instagram", color: "hover:bg-[#E4405F] hover:border-[#E4405F] hover:text-white" },
                                        { icon: Tiktok, key: "tiktok", color: "hover:bg-black hover:border-black hover:text-white" },
                                        { icon: Twitter, key: "twitter_x", color: "hover:bg-[#000000] hover:border-[#000000] hover:text-white" },
                                        { icon: Linkedin, key: "linkedin", color: "hover:bg-[#0A66C2] hover:border-[#0A66C2] hover:text-white" },
                                        { icon: Pinterest, key: "pinterest", color: "hover:bg-[#E60023] hover:border-[#E60023] hover:text-white" },
                                        { icon: Youtube, key: "youtube", color: "hover:bg-[#FF0000] hover:border-[#FF0000] hover:text-white" },
                                    ].map((social) => {
                                        const url = (business as any)[social.key];
                                        if (!url || url === "" || url === "#") return null;
                                        return (
                                            <Link key={social.key} href={url as string} target="_blank">
                                                <div className={`w-12 h-12 rounded-xl bg-green-xpale text-green flex items-center justify-center transition-all duration-300 hover:text-white ${social.color} hover:shadow-xl transform hover:-translate-y-1 border border-green-pale`}>
                                                    <social.icon size={20} />
                                                </div>
                                            </Link>
                                        );
                            ...
                            <div className="bg-white rounded-[1.5rem] overflow-hidden border border-border shadow-2xl">
                                <div className="bg-green-deeper p-6 flex items-center justify-between">
                                    <div className="space-y-0.5">
                                        <h3 className="text-white font-outfit font-black text-lg uppercase tracking-widest">Horario</h3>
                                        <p className="text-green-light text-[9px] font-bold uppercase tracking-[0.2em]">San Juan del Río</p>
                                    </div>
                                ...
                                <div className="p-6 space-y-3">
                                    {Object.entries(business.schedule || {}).map(([day, sch]: [string, any]) => (
                                        <div key={day} className={cn(
                                            "flex justify-between items-center py-1.5 transition-all px-3 rounded-lg",
                                            day === days[new Date().getDay()] ? "bg-green-xpale text-green shadow-inner" : "text-muted hover:bg-black/[0.02]"
                                        )}>
                                            <span className="uppercase font-black text-[10px] tracking-widest">{day}</span>
                                            <span className="text-xs font-bold font-jakarta">
                                                {sch.closed ? <span className="opacity-40 italic">Cerrado</span> : `${sch.open} - ${sch.close} hrs`}
                                            </span>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <section className="bg-[#F4FBF5] rounded-[1.5rem] p-12 border border-border/50">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                                    {business.products.map((product: any, i: number) => (
                                        <div key={i} className="bg-white p-8 rounded-[1rem] border border-border/50 shadow-sm group hover:border-green-mid transition-all hover:-translate-y-1">
                                            <div className="flex justify-between items-start mb-4">
                                                <h3 className="font-outfit font-black text-lg text-ink leading-tight group-hover:text-green-mid transition-all">{product.name}</h3>
                                                <span className="text-green font-black text-lg leading-tight bg-green-xpale px-3 py-1 rounded-xl shadow-inner">{product.price}</span>
                                            </div>
                                            <p className="text-muted text-sm font-jakarta leading-relaxed">{product.description}</p>
                                        </div>
                                    ))}
                                </div>
                            </section>
                    ...
                    <aside className="lg:col-span-1 space-y-10">
                        ...
                        <div className="bg-white rounded-[1.5rem] p-10 border border-border shadow-xl space-y-10">
                            <h3 className="font-outfit font-black text-xl text-green-deeper uppercase tracking-[0.2em]">Contacto Directo</h3>
                            <div className="space-y-8">
                                {business.phone && (
                                    <div className="flex items-start gap-5 group">
                                        ...
                                        <div>
                                            <p className="text-[10px] text-muted font-black uppercase tracking-widest mb-1">Teléfono principal</p>
                                            <p className="font-jakarta font-black text-ink2 text-lg">{business.phone}</p>
                                        </div>
                                    </div>
                                )}
                                {business.email && (
                                    <a href={`mailto:${business.email}`} className="flex items-start gap-5 group">
                                        <div className="w-12 h-12 rounded-[0.75rem] bg-green-xpale text-green flex items-center justify-center flex-shrink-0 group-hover:bg-green group-hover:text-white transition-all shadow-sm">
                                            <Mail size={20} />
                                        </div>
                                        <div className="overflow-hidden">
                                            <p className="text-[10px] text-muted font-black uppercase tracking-widest mb-1 group-hover:text-green transition-colors">Enviar mensaje</p>
                                            <p className="font-jakarta font-black text-ink2 truncate">{business.email}</p>
                                        </div>
                                    </a>
                                )}
                                {business.address && (
                                    <div className="flex items-start gap-5 group">
                                        ...
                                        <div>
                                            <p className="text-[10px] text-muted font-black uppercase tracking-widest mb-1">Dirección oficial</p>
                        ...
                        <div className="bg-green p-10 rounded-[1.5rem] flex flex-col items-center text-center text-white shadow-[0_10px_30px_rgba(42,122,59,0.3)] border-4 border-white/20">
                            ...
                            <h3 className="font-outfit font-black text-xl mb-4 leading-tight">¡Apoya a este negocio local!</h3>
                            <p className="text-white/70 text-xs font-jakarta mb-8 leading-relaxed">Comparte este micrositio con tus amigos y ayúdanos a fortalecer nuestra comunidad en San Juan del Río.</p>
                            <div className="flex gap-4 w-full">
                                <Link href={`https://wa.me/?text=Mira+el+micrositio+de+${encodeURIComponent(business.name)}+en+Enlace+San+Juan:+${encodeURIComponent("https://enlacesanjuan.com.mx/" + business.slug)}`} className="flex-1">
                                    <Button size="lg" className="w-full bg-white text-green hover:bg-green-xpale rounded-xl font-black uppercase text-[10px] tracking-widest shadow-xl border-none">Compartir</Button>
                                </Link>
                            </div>
                        </div>
          ...
          <div className="lg:col-span-2 space-y-16">
            ...
            <section className="relative rounded-[1.5rem] overflow-hidden bg-white shadow-2xl border border-border group">
              ...
              <div className="px-6 md:px-8 pb-2 md:pb-4 -mt-28 md:-mt-36 relative z-10 flex flex-col md:flex-row items-center md:items-end gap-5 md:gap-8 text-center md:text-left">
                {/* Logo - REDONDO COMPLETAMENTE */}
                <div className="w-36 h-36 md:w-44 md:h-44 rounded-full bg-white border-4 border-white shadow-2xl overflow-hidden flex-shrink-0 relative group mx-auto md:mx-0 md:mb-[42px]">
                  <div className="relative w-full h-full bg-white flex items-center justify-center rounded-full overflow-hidden">
                    {business.logo_url ? (
                      <Image
                ...
                <div className="flex-1 pb-2 md:pb-4 transition-all duration-500 relative">
                  ...
                  <div className="mb-4 md:mb-0 relative w-full pt-4 md:pt-0">
                     <h1 
                        className={cn(
                          "font-outfit font-black text-ink md:text-white leading-[0.95] drop-shadow-2xl transition-all duration-500 mx-auto md:mx-0",
