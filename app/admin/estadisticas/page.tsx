...
export default async function EstadisticasPage() {
  ...
  // Obtener visitas de negocios (páginas internas)
  const { data: businesses } = await supabase
    .from('businesses')
    .select('name, slug, views, category:categories(name)')
    .order('views', { ascending: false })
    .limit(50);

  const businessViews = (businesses || []).map((b: any) => ({
    ...b,
  ...
  return (
    <AdminLayout>
      <div className="space-y-12 animate-in fade-in duration-700">
        ...
        {/* Listado Detallado (Ediciones y Negocios) */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-[2.5rem] border border-border shadow-xl overflow-hidden flex flex-col">
            <div className="p-6 border-b border-border bg-green-xpale/10 flex items-center gap-3">
              ...
              <h2 className="font-outfit font-black text-sm text-ink uppercase tracking-widest">Impacto por Edición (Revista)</h2>
            </div>
          ...
          {/* Tabla de Páginas Internas (Micrositios de Negocios) */}
          <div className="bg-white rounded-[2.5rem] border border-border shadow-xl overflow-hidden flex flex-col">
            <div className="p-6 border-b border-border bg-green-xpale/10 flex items-center gap-3">
              <Building2 size={18} className="text-green" />
              <h2 className="font-outfit font-black text-sm text-ink uppercase tracking-widest">Páginas Internas (Negocios)</h2>
            </div>
            <div className="flex-1 overflow-y-auto max-h-[500px]">
              <table className="w-full">
                <thead className="sticky top-0 bg-white shadow-sm z-10">
                ...
                <tbody className="divide-y divide-border/50">
                  {businessViews.length === 0 ? (
                    <tr>
                      <td colSpan={3} className="px-6 py-10 text-center text-[10px] font-bold text-muted uppercase tracking-widest">
                        Sin negocios registrados
                      </td>
                    </tr>
                  ) : (
                    businessViews.map((biz: any) => (
                      <tr key={biz.slug} className="hover:bg-green-xpale/5 transition-colors">
                        <td className="px-6 py-4 font-outfit font-black text-ink text-xs uppercase">
                          {biz.name}
                        </td>
                        <td className="px-6 py-4 text-left font-jakarta text-[10px] text-muted uppercase font-bold tracking-wider">
                          {biz.category?.name || 'General'}
                        </td>
                        <td className="px-6 py-4 text-right font-outfit font-black text-green text-sm">
                          {biz.views || 0}
                        </td>
                      </tr>
                    ))
