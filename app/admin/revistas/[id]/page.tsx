...
import { Copy, UploadCloud, Link as LinkIcon, Trash2, Eye, ExternalLink, Loader2, Save } from "lucide-react";
...
export default function EditMagazinePage({ params }: { params: { id: string } }) {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, pageNum: number) => {
        ...
        try {
            ...
            if (existingPage) {
                ...
                const { error: insertError } = await supabase
                    .from("magazine_pages")
                    .insert([{ magazine_id: params.id, page_number: pageNum, image_url: publicUrl }]);
    ...
    const handlePageDetailUpdate = async (pageNum: number, field: "business_name" | "business_link", value: string) => {
        const existingPage = pages.find(p => p.page_number === pageNum);
        if (!existingPage) {
        ...
        try {
            ...
            const updatedPages = pages.map(p => p.id === existingPage.id ? { ...p, [field]: value } : p);
    ...
    if (loading) return <div className="flex justify-center items-center h-screen"><Loader2 className="w-8 h-8 animate-spin text-green" /></div>;
    ...
    return (
        <div className="p-8 max-w-[1600px] mx-auto space-y-8 pb-32 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between bg-white p-6 rounded-3xl border border-border shadow-sm">
                <div>
                    <h1 className="font-outfit font-black text-2xl text-ink uppercase">Editor de Revista</h1>
                    <p className="text-muted text-xs font-jakarta mt-1 tracking-widest">SUBE IMÁGENES EN ALTA 9:16 (Baja resolución recomendada para web)</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4">
                    <input 
                        type="text"
                        value={month} 
                        onChange={e => setMonth(e.target.value.toLowerCase().replace(/[^a-z0-9]/g, "-").replace(/-+/g, "-"))}
                    ...
                    <Button onClick={handleSaveMagazineInfo} disabled={saving} className="h-12 bg-ink hover:bg-black rounded-xl">
                        {saving ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Save className="w-4 h-4 mr-2" />}
                        Configurar
                    </Button>

                    <a href={`/revista/${year}/${month}`} target="_blank" rel="noopener noreferrer">
            ...
            {status === "published" && (
                <div className="bg-green-xpale border border-green-pale p-4 rounded-2xl flex items-center justify-between">
                    <div>
                        <p className="text-xs font-bold text-green uppercase tracking-widest mb-1">Enlace de publicación activo</p>
                        <a href={`/revista/${year}/${month}`} target="_blank" className="font-mono text-sm text-green-deeper font-black">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 gap-6">
                {allPages.map(pageNum => {
                    ...
                    return (
                        <div key={pageNum} className="bg-white rounded-2xl border-2 border-border border-dashed hover:border-green-pale transition-all p-4 flex flex-col group relative">
                            ...
                            <div className="flex items-center justify-between mb-3">
                                <span className={`text-[10px] font-black uppercase tracking-widest ${isCover || isBackCover ? 'text-green' : 'text-muted'}`}>{label}</span>
                                {pageData && (
                                    <button onClick={() => handleDeletePage(pageNum)} className="text-red-400 hover:text-red-600 transition-colors">
                            ...
                            <div className="relative aspect-[9/16] bg-gray-50 rounded-xl overflow-hidden mb-4 border border-border group-hover:shadow-md transition-shadow">
                                {pageData?.image_url ? (
                                    <Image src={pageData.image_url} alt={`Page ${pageNum}`} fill className="object-cover" />
                                ) : (
                                    <div className="absolute inset-0 flex flex-col items-center justify-center text-muted gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-widest text-center px-4">Subir Imagen JPG 9:16</span>
                                    </div>
                                )}
                            ...
                            <div className="flex flex-col gap-2 mt-auto">
                                <input 
                                    ...
                                    onBlur={(e) => handlePageDetailUpdate(pageNum, "business_name", e.target.value)}
                                />
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        onBlur={(e) => handlePageDetailUpdate(pageNum, "business_link", e.target.value)}
                                    />
                                </div>
                            </div>
                        </div>
