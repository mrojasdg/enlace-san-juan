...
<<<<<<< HEAD
    ...
    useEffect(() => {
        const fetchBusinesses = async () => {
            ...
            let query = supabase
                .from("businesses")
                .select("id, name, slug, logo_url, is_active, verified, is_featured, in_magazine, category_id, created_at, views, category:categories(name)", { count: "exact" });
        ...
        <AdminLayout>
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-12">
                <div className="max-w-xl">
                    <h2 className="font-outfit font-black text-4xl text-green-deeper leading-tight mb-4">Gestión de <br /> Negocios</h2>
                    <p className="text-muted font-jakarta text-sm">Administra, edita o elimina los establecimientos registrados en el directorio municipal.</p>
                </div>
            ...
            <div className="bg-white rounded-[3rem] border border-border overflow-hidden shadow-2xl">
                    ...
                    <table className="w-full text-left border-collapse min-w-[1200px]">
                        <thead>
                            <tr className="bg-[#F4FBF5] border-b border-border">
                                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted/60">Negocio & Info</th>
                                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted/60">Categoría</th>
                                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted/60">Info & Tráfico</th>
                                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted/60 text-center">Estado de Visibilidad</th>
                                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-muted/60 text-right">Acciones y Recursos</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-border">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={5} className="px-10 py-20 text-center text-muted font-jakarta italic text-sm">
                                        No se encontraron negocios con esos criterios.
                                    </td>
                                </tr>
                            ) : (
                                businesses?.map((biz) => (
                                    <tr key={biz.id} className="group hover:bg-green-xpale/30 transition-all duration-300">
                                            ...
                                            <div className="flex items-center gap-6">
                                                <div className="w-16 h-16 rounded-full bg-white border border-border shadow-md p-1 relative overflow-hidden flex-shrink-0 group-hover:shadow-xl transition-all">
                                                    {biz.logo_url ? (
                                                        ...
                                                        <div className="w-full h-full flex items-center justify-center font-black text-green-xpale text-xl">{biz.name[0]}</div>
                                                    )}
                                                </div>
                                                <div>
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <p className="font-outfit font-black text-[15px] text-ink leading-none">{biz.name}</p>
                                                        {biz.verified && <CheckCircle size={14} className="text-blue-500" />}
                                                    </div>
                                                    <p className="text-[10px] font-black text-muted/40 uppercase tracking-widest leading-none">/{biz.slug}</p>
                                                    <div className="mt-3 flex items-center gap-4 text-[10px] font-black text-green/50 uppercase tracking-widest">
                                                        <span className="flex items-center gap-1"><Star size={10} /> {biz.is_featured ? "Featured" : "Regular"}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-8 py-8">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-ink font-black text-lg font-outfit leading-none">{biz.views || 0}</p>
                                                    <p className="text-[9px] text-muted font-black uppercase tracking-widest">Visitas Acum.</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-8 py-8">
                                            <div className="flex items-center justify-end gap-2">
                                                ...
                                                <Link href={`/${biz.slug}`} target="_blank">
                                                    <button title="Ver Micrositio" className="w-12 h-12 rounded-2xl bg-white border border-border text-muted hover:text-green-mid hover:bg-green-xpale transition-all flex items-center justify-center shadow-sm">
                                                        <Eye size={18} />
                                                    </button>
                                                </Link>
                                                <Link href={`/admin/negocios/${biz.id}`}>
                                                    <button title="Editar" className="w-12 h-12 rounded-2xl bg-green-xpale text-green flex items-center justify-center border border-green-pale hover:bg-green hover:text-white transition-all shadow-sm">
                                                        <Edit size={18} />
                                                    </button>
                                                </Link>
                                                {/* Botón Eliminar (Simulado) */}
                                                <button title="Eliminar" className="w-12 h-12 rounded-2xl bg-red-50 text-red-400 flex items-center justify-center border border-red-100 hover:bg-red-500 hover:text-white transition-all shadow-sm">
                                                    <Trash2 size={18} />
                                                </button>
                                            </div>
                                        </td>
                ...
                <div className="bg-[#F4FBF5]/50 px-10 py-8 border-t border-border flex flex-col md:flex-row items-center justify-between gap-6">
                    <p className="font-jakarta text-[11px] font-bold text-muted/60 uppercase tracking-[0.1em]">
                        Mostrando {businesses?.length || 0} de {count || 0} negocios registrados
                    </p>
                    <div className="flex items-center gap-3">
                        <Link href={`/admin/negocios?page=${Math.max(1, page - 1)}&q=${searchTerm}`}>
                            <Button variant="outline" className="h-12 px-6 rounded-xl border-border font-black text-[10px] uppercase tracking-widest text-muted hover:text-green transition-all" disabled={page <= 1}>
                                Anterior
                            </Button>
                        </Link>
                        <div className="flex items-center gap-1 px-4">
                            <span className="font-black text-xs text-ink">{page}</span>
                            <span className="text-[10px] font-bold text-muted/40 uppercase tracking-widest">de {totalPages}</span>
                        </div>
                        <Link href={`/admin/negocios?page=${Math.min(totalPages, page + 1)}&q=${searchTerm}`}>
                            <Button variant="outline" className="h-12 px-6 rounded-xl border-border font-black text-[10px] uppercase tracking-widest text-muted hover:text-green transition-all" disabled={page >= totalPages}>
                                Siguiente
                            </Button>
                        </Link>
                    </div>
